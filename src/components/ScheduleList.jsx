...
const ScheduleList = ({ schedules = [], onEdit, onDelete }) => {
  ...
  const [sortConfig, setSortConfig] = useState({ key: 'date', direction: 'desc' }); // Default: Date Descending

  const handleSort = (key) => {
    let direction = 'asc';
    // If clicking the same key, toggle. If new key, default to ascending (except date usually starts desc)
                ...
                {[
                  ...
                  >
                    <div className="flex items-center gap-1">
                      {col.l}
                      {col.k !== 'actions' && <ArrowUpDown size={12} />}
                    </div>
                  </th>
                ))}
              </tr>
                  ...
                  <tr key={event.id} className="hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => onEdit(event)} className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-blue-400 transition-colors">
                          <Edit2 size={16} />
                        </button>
                        <button onClick={() => onDelete(event.id)} className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-red-400 transition-colors">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
